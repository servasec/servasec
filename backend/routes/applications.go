...
func RegisterApplicationRoutes(router *gin.Engine) {
	...
	{
		slug.GET("", middleware.CheckPolicy("/applications/*", "read"), controllers.GetApplication)
		slug.POST("/ingest", middleware.CheckPolicy("/applications/*", "write"), controllers.IngestScan)
		slug.GET("/versions", middleware.CheckPolicy("/applications/*", "read"), controllers.GetVersions)
		slug.POST("/versions", middleware.CheckPolicy("/applications/*", "write"), controllers.CreateVersion)
		slug.GET("/versions/compare", middleware.CheckPolicy("/applications/*", "read"), controllers.CompareVersions)
		slug.GET("/versions/:versionId", middleware.CheckPolicy("/applications/*", "read"), controllers.GetVersion)
		slug.PATCH("/versions/:versionId", middleware.CheckPolicy("/applications/*", "write"), controllers.UpdateVersion)
		slug.DELETE("/versions/:versionId", middleware.CheckPolicy("/applications/*", "write"), controllers.DeleteVersion)
		slug.GET("/versions/:versionId/findings", middleware.CheckPolicy("/applications/*", "read"), controllers.GetVersionFindings)
	}
