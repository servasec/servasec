...
func RegisterFindingRoutes(router *gin.Engine) {
	f := router.Group("/findings", middleware.AuthRequired())
	{
		...
		f.PATCH("/:id/assign", middleware.CheckPolicy("/findings/*", "write"), controllers.AssignFinding)
		f.PATCH("/:id/review", middleware.CheckPolicy("/findings/*", "write"), controllers.ReviewFinding)
		f.POST("/:id/comments", middleware.CheckPolicy("/findings/*", "write"), controllers.CreateComment)
		f.GET("/:id/comments", middleware.CheckPolicy("/findings/*", "read"), controllers.GetComments)
	}
}
